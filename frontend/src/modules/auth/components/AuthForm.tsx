...
import { login, type LoginData } from "../services/logInService";
import { registerUser, type RegisterData } from "../services/signUpService";
    ...
    const handleSubmit = async (e: FormEvent) => {
        ...
        try {
            if (mode === "login") {
                ...
                const data = await login({ email, password } as LoginData);
            ...
            setError(err.response?.data?.detail || err.message || "Сталася помилка");
        } finally {
            setLoading(false);
        }
    };
            ...
            <div className="w-full max-w-md bg-[#e4fbe1] p-10 rounded-3xl shadow-md">
                <div className="text-center mb-8">
                    <h1 className="text-3xl font-bold mb-3">
                        {mode === "login" ? "Вхід до акаунту" : "Створіть акаунт"}
                    </h1>
